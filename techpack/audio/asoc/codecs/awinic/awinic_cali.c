...
#define AWINIC_CALI_FILE  "/mnt/vendor/persist/engineermode/aw_cali.bin"
...
static ssize_t aw882xx_cali_re_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int32_t local_re = 0;
	int ret;
	...
	ret = aw_write_data_to_dsp(INDEX_PARAMS_ID_RX_RE, (void *)&local_re,
...
#ifdef IGNORE_CALI_RA
	aw882xx->cali.re = aw882xx->cali.re - aw882xx->cali.ra;
#endif

	len = snprintf(buf, PAGE_SIZE, "2:%d,2:0", aw882xx->cali.re);
...
static DEVICE_ATTR(calibra, S_IWUSR | S_IRUGO,
...
/*****************************************************
 *
 * device tree parse cali ra
 *
 *****************************************************/
#ifdef IGNORE_CALI_RA
void aw882xx_parse_cali_ra_dt(struct aw_cali *cali)
{
	int ret = -1;
	struct aw882xx *aw882xx =
			container_of(cali, struct aw882xx, cali);
	...
	ret = of_property_read_u32(np, "cali-ra-val", &cali->ra);
	if (ret) {
		cali->ra = 0;
		dev_err(aw882xx->dev,
			"%s: cali->ra get failed ,user zero value!\n",
			__func__);
	} else {
		dev_info(aw882xx->dev, "%s: cali-ra-val = %d\n",
			__func__, cali->ra);
	}
}
#endif
